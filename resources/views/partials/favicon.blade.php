@php
    ...
    if (session()->has('owner_id')) {
        $faviconOwner = \App\Models\Owner::find(session('owner_id'));
    } elseif (session()->has('user_id')) {
        $faviconUser = \App\Models\User::find(session('user_id'));
        $faviconOwner = $faviconUser ? \App\Models\Owner::find($faviconUser->id_owners) : null;
    }
    ...
    $faviconSvg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">'
        ...
        .'<text x="50" y="54" font-family="Arial, Helvetica, sans-serif" font-size="56" font-weight="800" fill="#ffffff" text-anchor="middle" dominant-baseline="middle">'.$faviconInitial.'</text>'
        .'</svg>';
@endphp
