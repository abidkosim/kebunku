<x-dynamic-component :component="$actorType === 'owner' ? 'owner.shell' : 'staff.shell'" :owner="$owner" active="tanaman-kelola" :logs="$logs" :actor-type="$actorType" :actor-nama="$actorNama" :actor-foto-url="$actorFotoUrl">
                        ...
                        <tr class="group transition-all duration-200 dark:hover:bg-slate-800/30 cursor-pointer" wire:click="viewDetail({{ $item->id }})">
                            ...
                            <td class="px-6 py-4 align-middle" onclick="event.stopPropagation()">
                                <div class="flex justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                    ...
                                    <button wire:click="deleteTanaman({{ $item->id }})" wire:confirm="Yakin hapus tanaman {{ $item->nama_tanaman }}? Semua tahap & jadwalnya juga akan terhapus." aria-label="Hapus tanaman" class="action-btn delete w-8 h-8 rounded-full bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 hover:bg-red-500 hover:text-white hover:border-red-500 shadow-sm flex items-center justify-center transition-all dark:text-slate-300">✕</button>
                                </div>
                            </td>
                        </tr>
                        @empty
        ...
        <div class="md:hidden divide-y divide-slate-100/70 dark:divide-slate-700/50">
            ...
            <div class="p-5 flex items-center justify-between gap-4 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition cursor-pointer" wire:click="viewDetail({{ $item->id }})">
                ...
                <div class="flex gap-2 shrink-0" onclick="event.stopPropagation()">
                    <button wire:click="openEditTanaman({{ $item->id }})" aria-label="Edit" class="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center hover:bg-slate-200 dark:hover:bg-slate-600 transition dark:text-slate-300">✎</button>
                    <button wire:click="deleteTanaman({{ $item->id }})" wire:confirm="Hapus?" aria-label="Hapus" class="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center hover:bg-red-100 dark:hover:bg-red-900/30 hover:text-red-500 dark:hover:text-red-400 transition dark:text-slate-300">✕</button>
                </div>
            </div>
            @empty
    ...
    <div class="glass-card rounded-2xl shadow-lg shadow-slate-200/50 dark:shadow-slate-800/20 p-6 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex gap-2">
            <button wire:click="openEditTanaman({{ $selected->id }})" class="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center hover:bg-slate-200 dark:hover:bg-slate-600 transition dark:text-slate-300">✎</button>
            <button wire:click="deleteTanaman({{ $selected->id }})" wire:confirm="Yakin hapus tanaman ini? Semua tahap & jadwalnya juga akan terhapus." class="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center hover:bg-red-100 dark:hover:bg-red-900/30 hover:text-red-500 dark:hover:text-red-400 transition dark:text-slate-300">✕</button>
        </div>
    </div>
