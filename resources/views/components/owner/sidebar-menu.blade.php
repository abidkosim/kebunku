...
@foreach($menus as $menu)
    ...
    @else
        <a href="{{ $menu['href'] }}" class="sidebar-link {{ $active === $menu['slug'] ? 'active' : 'text-slate-600 dark:text-slate-300' }} flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $menu['icon'] }}"/></svg>
            {{ $menu['label'] }}
        </a>
    @endif
