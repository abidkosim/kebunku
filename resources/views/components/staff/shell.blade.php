        ...
        <div class="px-4 h-16 flex items-center justify-between gap-3">
            <div class="relative min-w-0" x-data="{ userMenuOpen: false }">
                ...
                <div x-show="userMenuOpen" @click.outside="userMenuOpen = false" x-transition
                     ...
                     style="display:none;">
                    <a href="{{ route('portal.akun') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition">
            ...
            <div class="flex items-center gap-2 shrink-0 ml-auto">
                <div class="relative" x-data="{ notifOpen: false }">
                    <button @click="notifOpen = !notifOpen" aria-label="Notifikasi" class="w-9 h-9 rounded-full bg-white/80 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center relative">
    ...
    <nav class="fixed bottom-0 inset-x-0 z-30 glass-card border-t border-slate-200/60 dark:border-slate-700/50">
        <div class="max-w-lg mx-auto grid gap-1 px-2 py-2" style="grid-template-columns: repeat({{ count($nav) }}, minmax(0, 1fr));">
            @foreach($nav as $item)
                ...
                <a href="{{ $item['href'] }}" class="flex flex-col items-center gap-1 py-1.5 rounded-xl transition {{ $isActive ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-500' }}">
